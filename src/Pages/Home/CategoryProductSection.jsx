...
import { FiShoppingCart, FiFilter, FiX, FiMapPin, FiCheckCircle, FiStar } from "react-icons/fi";
import { Link } from "react-router-dom";
  ...
  return (
    <div className="relative p-3 sm:p-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {finalProducts.length > 0 ? (
          finalProducts.map((product) => (
            <div key={product.id} className="bg-white border rounded-lg shadow-sm hover:shadow-lg transition-all overflow-hidden flex flex-col cursor-pointer">
              <div
              ...
              >
                ...
                />
                {product.discount && (
                  <span className="absolute top-2 right-2 inline-flex items-center justify-center rounded-full border px-2 py-0.5 text-xs font-medium text-white bg-red-500 hover:bg-red-600">
                    {product.discount}% ছাড়
                  </span>
                )}
              </div>
              ...
              <div className="p-3 space-y-2">
                ...
                <div className="flex items-center justify-between text-xs sm:text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    {product.verified && <FiCheckCircle className="w-4 h-4 text-blue-500" title="যাচাইকৃত বিক্রেতা" />}
                  </div>
                </div>

                <div className="flex items-center gap-2 pt-1">
                  <span className="text-xs sm:text-sm text-gray-600 flex-1">{product.seller || "অজানা বিক্রেতা"}</span>
                  <button
          ...
          <p className="text-center py-10 text-gray-500">এই ক্যাটেগরিতে কোনো পণ্য পাওয়া যায়নি 😔</p>
        )}
      </div>

      {/* Filter Drawer */}
      <div className={`fixed top-0 right-0 h-full w-72 sm:w-80 bg-white shadow-lg transform transition-transform duration-300 z-50 ${isFilterOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="flex justify-between items-center px-4 py-3 border-b">
          <h3 className="text-lg font-bold">ফিল্টার অপশন</h3>
        ...
        <div className="p-4 space-y-5 overflow-y-auto h-[calc(100%-60px)]">
          <div>
            <h4 className="font-semibold mb-2">মূল্য পরিসীমা</h4>
            <input type="range" min="0" max="50000" value={priceRange} onChange={(e) => setPriceRange(Number(e.target.value))} className="w-full accent-black" />
            <div className="flex justify-between text-sm text-gray-600">
              <span>৳0</span>
              <span>৳{priceRange}</span>
            </div>
          ...
          <div>
            <h4 className="font-semibold mb-2">ন্যূনতম রেটিং</h4>
            <input type="range" min="0" max="5" step="0.5" value={rating} onChange={(e) => setRating(Number(e.target.value))} className="w-full accent-black" />
            <p className="text-sm text-gray-600">⭐ {rating} এবং তার বেশি</p>
          </div>
          ...
          <button onClick={() => setIsFilterOpen(false)} className="w-full bg-black text-white py-2 rounded hover:bg-gray-800 transition text-sm">
            সব ফিল্টার প্রয়োগ করুন
          </button>
        </div>
      </div>
      ...
      {/* Overlay */}
      {isFilterOpen && <div onClick={() => setIsFilterOpen(false)} className="fixed inset-0 bg-black/40 backdrop-blur-sm z-40" />}
