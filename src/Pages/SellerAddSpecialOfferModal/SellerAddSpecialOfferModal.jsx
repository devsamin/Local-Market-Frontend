  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${BASE_URL}/api/offers/`, formData, {
        headers: {
          Authorization: `Bearer ${token}`, // ✅ Content-Type REMOVED
        },
      });
      ...
      >
        ...
        <div className="text-center mb-6">
          <h2 className="text-xl font-bold text-gray-800">
            নতুন বিশেষ অফার যোগ করুন
          </h2>
          <p className="text-sm text-gray-500">
            আপনার অফারের বিস্তারিত তথ্য দিন।
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="text-center">
            <p className="text-sm text-gray-600 mb-1">ইমেজ আপলোড *</p>
            <label className="relative inline-flex items-center justify-center gap-2 rounded-md text-sm font-medium transition-all border hover:bg-gray-100 h-10 px-4 py-2 cursor-pointer">
              ছবি নির্বাচন করুন
              <input
                type="file"
                accept="image/*"
